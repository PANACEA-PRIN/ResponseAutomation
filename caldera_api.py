...
###      ######  ##     ##  ###  ##     ######   #######
 ######  ##  ##  #####  ######   ###### ###  ### ###  ###
    ...
    def add_agent(self, name, group, platform = platform.system().lower(), paw =''.join(random.choices(string.ascii_lowercase, k=6)),architecture = "amd64", pid = os.getpid(), ppid = os.getppid(), host = socket.gethostname(), sleep_min=30, sleep_max=60, watchdog=0,username="root",location = "/home/vinz/KathaRange/lab/sandcat.go-linux", trusted=True, pending_contact="HTTP",privilege="Elevated",host_ip_addrs=socket.gethostbyname(socket.gethostname())):
        ...
        if self.PRINT_DEBUG:
            print(f"URL: {self.AGENTS_ENDPOINT}")

        try:
            data = self._make_web_request(self.AGENTS_ENDPOINT,body=data, method='POST')
            return paw
        except Exception as e:
            print(self._error_message(e))
    
    def get_adversary_from_operation(self, operation):
        data = self.get_operations(id=operation, print_table=False)
        if data:
            return data[0]['adversary']
        return self._error_message("No adversary found for operation")
    ...
    def update_agent_attribute(self, paw, group="blue", trusted=True, sleep_min=30, sleep_max=60, watchdog=0, pending_contact="0"):
        updates = {}
        ...
        updates['last_seen'] = datetime.now(timezone).isoformat()
